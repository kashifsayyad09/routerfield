...
import { loadImageModels, imageToImageModels } from '../lib/models.js';
import { ENHANCE_TAGS, QUICK_PROMPTS } from '../lib/promptUtils.js';
import { AuthModal } from './AuthModal.js';
import { createUploadPicker } from './UploadPicker.js';

// Reference images: OpenRouter's `input_references` accepts more than one
// image on providers that support it, but discovery doesn't publish a
// per-model maximum — we cap client-side at a sane default.
...
export function ImageStudio() {
    ...
    const findModel = (id) => (imageMode ? i2iModels : t2iModels).find(m => m.id === id)
        || t2iModels.find(m => m.id === id) || i2iModels.find(m => m.id === id) || null;
    const getCurrentAspectRatios = (id) => findModel(id)?.aspectRatios || [];
    const getCurrentResolutions = (id) => {
        const m = findModel(id);
        return m?.resolutions?.length ? m.resolutions : (m?.qualities || []);
    };
    const getCurrentQualityField = (id) => {
        const m = findModel(id);
        return m?.resolutions?.length ? 'resolution' : 'quality';
    };
    ...
    const picker = createUploadPicker({
        ...
        onSelect: ({ url, urls }) => {
            ...
            if (!imageMode && i2iModels.length > 0) {
                ...
                picker.setMaxImages(MAX_REFERENCE_IMAGES);
            }
        ...
        onClear: () => {
            ...
            if (t2iModels.length > 0) {
                ...
                document.getElementById('model-btn-label').textContent = selectedModelName;
                document.getElementById('ar-btn-label').textContent = selectedAr;
                const t2iResolutions = getCurrentResolutions(selectedModel);
    ...
    // Quality button visibility/label is set once models finish loading (see loadModelsAsync below).
    ...
    modelLoadBanner.innerHTML = `<span class="animate-spin inline-block">◌</span> Loading image models from OpenRouter…`;
    ...
    const applyDefaultModel = () => {
        ...
        if (resolutions.length > 0) document.getElementById('quality-btn-label').textContent = resolutions[0];
    };

    loadImageModels()
        .then((models) => {
            t2iModels = models;
            i2iModels = imageToImageModels(models);
            modelsLoaded = true;
            applyDefaultModel();
        ...
        .catch((err) => {
            modelsLoadError = err.message || 'Could not load image models from OpenRouter.';
            ...
            modelLoadBanner.textContent = `Couldn't load models from OpenRouter: ${modelsLoadError}`;
        });
    ...
    const addToHistory = (entry) => {
        ...
        localStorage.setItem('openrouter_history', JSON.stringify(generationHistory.slice(0, 50)));
    ...
    try {
        const saved = JSON.parse(localStorage.getItem('openrouter_history') || '[]');
        if (saved.length > 0) {
    ...
    // Note: the old MuAPI integration was asynchronous (submit → request_id →
    // poll) and could leave a "pending job" behind if the tab closed mid-poll,
    // which the app resumed on next load. OpenRouter's Image API responds
    // synchronously in a single request, so there's no pending-job state to
    // resume here anymore.
    ...
    downloadBtn.onclick = () => {
        ...
        if (current) {
            ...
            downloadImage(current, `openrouter-${entry?.id || 'image'}.jpg`);
        }
    };
    ...
    generateBtn.onclick = async () => {
        ...
        try {
            ...
            const genParams = {
                model: selectedModel,
                prompt: prompt || (imageMode ? 'Edit this image.' : ''),
            ...
            if (imageMode) genParams.referenceImages = uploadedImageUrls;

            const res = await openrouter.generateImage(genParams);
