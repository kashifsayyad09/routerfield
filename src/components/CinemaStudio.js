...
import { loadImageModels } from '../lib/models.js';
...
export function CinemaStudio() {
    ...
    loadImageModels()
        .then((models) => {
            ...
            imageModel = (preferred || models[0]).id;
        })
        .catch((err) => { modelsError = err.message || 'Could not load image models from OpenRouter.'; });
    ...
    generateBtn.onclick = async () => {
        ...
        if (!imageModel) {
            alert(modelsError ? `Couldn't load models from OpenRouter: ${modelsError}` : 'Image models are still loading from OpenRouter — try again in a moment.');
            return;
        }
        ...
        try {
            const res = await openrouter.generateImage({
                model: imageModel,
                prompt: finalPrompt,
