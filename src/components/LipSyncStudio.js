...
export function LipSyncStudio() {
    ...
    videoFileInput.onchange = async (e) => {
        ...
        showVideoSpinner();
        try {
            uploadedVideoUrl = await openrouter.uploadFile(file);
    ...
    audioFileInput.onchange = async (e) => {
        ...
        showAudioSpinner();
        try {
            uploadedAudioUrl = await openrouter.uploadFile(file);
    ...
    const getResolutionsForLipSyncModel = () => [];
    ...
    const updateUIForMode = () => {
        ...
        // No lip-sync models are available from OpenRouter (see notice below),
        // so there is nothing to select here — the resolution control stays hidden.
        resolutionBtn.classList.add('hidden');
    };
    ...
    // No lip-sync models are available from OpenRouter — keep the resolution control hidden.
    resolutionBtn.classList.add('hidden');
    ...
    // Note: lip-sync generation is disabled (see LIPSYNC_UNAVAILABLE_MESSAGE),
    // so there is nothing to resume here — the old MuAPI job-polling logic
    // has been removed rather than left pointing at a provider that no
    // longer exists.
    ...
    // ==========================================
    // 8. GENERATION LOGIC — disabled (see LIPSYNC_UNAVAILABLE_MESSAGE above)
    // ==========================================
    generateBtn.disabled = true;
    generateBtn.title = LIPSYNC_UNAVAILABLE_MESSAGE;
    generateBtn.classList.add('opacity-50', 'cursor-not-allowed');
    generateBtn.onclick = () => {
        alert(LIPSYNC_UNAVAILABLE_MESSAGE);
    };

    // Persistent notice explaining why generation is unavailable.
    const unavailableNotice = document.createElement('div');
    unavailableNotice.className = 'mx-2 px-4 py-3 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-amber-200 text-xs leading-relaxed';
    unavailableNotice.textContent = LIPSYNC_UNAVAILABLE_MESSAGE;
    bar.insertBefore(unavailableNotice, bottomRow);
