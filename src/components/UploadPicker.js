...
export function createUploadPicker({ anchorContainer, onSelect, onClear, maxImages: initialMaxImages = 1 }) {
    ...
    fileInput.onchange = async (e) => {
        ...
        // Validate before doing any work — OpenRouter's image/video APIs take
        // reference images inline, so there's no network round-trip here and
        // no need to gate this on having an API key yet (only generation needs one).
        const invalid = files.filter((f) => !ACCEPTED_IMAGE_TYPES.includes(f.type));
        if (invalid.length) {
            alert(`Unsupported file type: ${invalid.map((f) => f.name).join(', ')}. Please use PNG, JPEG, or WebP.`);
            fileInput.value = '';
            return;
        }
        const tooLarge = files.filter((f) => f.size > MAX_IMAGE_BYTES);
        if (tooLarge.length) {
            alert(`These images are too large (max ${MAX_IMAGE_BYTES / 1024 / 1024}MB): ${tooLarge.map((f) => f.name).join(', ')}`);
            fileInput.value = '';
            return;
        }
        ...
        try {
            if (maxImages === 1) {
                // Single mode: read first file only, replace selection
                const file = files[0];
                const [uploadedUrl, thumbnail] = await Promise.all([
                    openrouter.uploadFile(file),
                ...
                // Multi mode: read all files (up to remaining slots)
                const slots = maxImages - selectedEntries.length;
                const toUpload = files.slice(0, Math.max(slots, 1));

                // Read all in parallel — all local, no network involved.
