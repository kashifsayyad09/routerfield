...
import { generateImage } from "../lib/openrouter/client.js";
import { loadImageModels, imageToImageModels } from "../lib/openrouter/models.js";
import { prepareUploadedImage, UploadValidationError, MAX_IMAGE_BYTES } from "../lib/openrouter/upload.js";

// Reference images: OpenRouter's `input_references` array accepts more than
// one image on providers that support it, but the discovery API doesn't
// publish a per-model maximum — we cap client-side at a sane default.
...
function UploadButton({ maxImages, onSelect, onClear }) {
    ...
    try {
      ...
      const results = await Promise.allSettled(
        toUpload.map(async (file) => {
          // Validate + read locally (no network round-trip): OpenRouter's
          // image/video APIs accept a base64 data: URL directly as a
          // reference image, so there's no separate "upload" step or host.
          const prepared = await prepareUploadedImage(file);
          const id = Date.now().toString() + Math.random();
          const entry = { id, name: file.name, url: prepared.dataUrl, revoke: prepared.revoke, progress: 100 };
      ...
      if (failures.length) {
        const messages = failures.map((r) =>
          r.reason instanceof UploadValidationError ? r.reason.message : "Upload failed."
        );
        alert(messages.join("\n"));
      }
      ...
      succeeded.forEach((entry) => {
        if (selectedEntries.length < maxImages) {
          setSelectedEntries((prev) => [...prev, { url: entry.url }]);
          if (maxImages === 1) {
            fireOnSelect([{ url: entry.url }]);
            setPanelOpen(false);
          }
        }
      });
  ...
  const handleRemoveFromHistory = (e, entry) => {
    e.stopPropagation();
    entry.revoke?.(); // release the object URL created when this file was read
...
export default function ImageStudio({ onGenerationComplete, historyItems }) {
  // ── Model discovery ──────────────────────────────────────────────────────
  const [allModels, setAllModels] = useState([]); // normalized OpenRouter image models
  ...
  useEffect(() => {
    ...
    loadImageModels()
      ...
      .catch((err) => {
        if (cancelled) return;
        setModelsError(err.message || "Could not load image models from OpenRouter.");
      })
      .finally(() => !cancelled && setModelsLoading(false));
    return () => { cancelled = true; };
  }, []);

  const t2iModels = allModels; // every discovered image model supports text-to-image
  const i2iModels = useMemo(() => imageToImageModels(allModels), [allModels]);
  ...
  // Once models load, select the first text-to-image model by default.
  useEffect(() => {
    if (selectedModelId || t2iModels.length === 0) return;
    const first = t2iModels[0];
    setSelectedModelId(first.id);
    setSelectedModelName(first.name);
    setSelectedAr(first.aspectRatios?.[0] || "1:1");
    setSelectedQuality(first.resolutions?.[0] || first.qualities?.[0] || null);
  }, [t2iModels, selectedModelId]);
  ...
  const currentAspectRatios = selectedModel?.aspectRatios || [];
  // Some models expose a `resolution` tier (512/1K/2K/4K), others a `quality`
  // knob (auto/low/medium/high) — whichever the model supports drives this button.
  const currentResolutions = selectedModel?.resolutions?.length
    ? selectedModel.resolutions
    : selectedModel?.qualities || [];
  const currentQualityField = selectedModel?.resolutions?.length ? "resolution" : "quality";
  ...
  const handleUploadSelect = useCallback(
    ({ url, urls }) => {
      ...
      if (!imageMode && i2iModels.length > 0) {
        const firstI2I = i2iModels[0];
        setImageMode(true);
        setSelectedModelId(firstI2I.id);
        setSelectedModelName(firstI2I.name);
        setSelectedAr(firstI2I.aspectRatios?.[0] || "1:1");
        setSelectedQuality(firstI2I.resolutions?.[0] || firstI2I.qualities?.[0] || null);
        setMaxImages(MAX_REFERENCE_IMAGES);
      }
    },
    [imageMode, i2iModels]
  );
  ...
  const handleUploadClear = useCallback(() => {
    ...
    if (firstT2I) {
      ...
      setSelectedAr(firstT2I.aspectRatios?.[0] || "1:1");
      setSelectedQuality(firstT2I.resolutions?.[0] || firstT2I.qualities?.[0] || null);
    }
    setMaxImages(1);
  }, [t2iModels]);
  ...
  const handleModelSelect = (m) => {
    ...
    setSelectedQuality(m.resolutions?.[0] || m.qualities?.[0] || null);
    if (imageMode) setMaxImages(MAX_REFERENCE_IMAGES);
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      const genParams = {
        ...
        prompt: prompt.trim() || (imageMode ? "Edit this image." : ""),
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover/thumb:opacity-100 transition-opacity flex items-center justify-center gap-1">
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          downloadImage(entry.url, `openrouter-${entry.id || idx}.jpg`);
                        }}
          ...
          <div className="mt-6 flex gap-3 justify-center">
            ...
            <button
              ...
              onClick={() => {
                ...
                downloadImage(currentImageUrl, `openrouter-${entry?.id || "image"}.jpg`);
              }}
      ...
      {!showCanvas && (
        <>
          ...
          <div className="w-full max-w-4xl relative z-40 animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
            {modelsError && (
              <div className="mb-3 px-4 py-2.5 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-300 text-xs font-medium">
