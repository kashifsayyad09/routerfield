...
import { loadImageModels } from '../lib/openrouter/models.js';
...
export default function CinemaStudio({ onGenerationComplete, historyItems }) {
    // ── Model discovery ──
    // Cinema Studio always shoots stills with a single, high-quality image
    // model (the old build hard-coded MuAPI's "nano-banana-pro"). We prefer
    // an OpenRouter Nano Banana / Gemini image model if one is available,
    // and otherwise fall back to whatever image model OpenRouter lists
    // first — never a hard-coded id that might not exist on this account.
    ...
    useEffect(() => {
        ...
        loadImageModels()
            .then((models) => {
                ...
                const preferred = models.find((m) => /nano-banana|gemini/i.test(m.id) || /nano banana|gemini/i.test(m.name));
                setImageModel((preferred || models[0]).id);
            })
            .catch((err) => { if (!cancelled) setModelsError(err.message || 'Could not load image models from OpenRouter.'); });
    ...
    const handleGenerate = useCallback(async () => {
        ...
        if (!imageModel) {
            alert('Image models are still loading from OpenRouter — try again in a moment.');
            return;
        }
    ...
    }, [settings, resolution, imageModel, isGenerating, onGenerationComplete, historyItems]);
        ...
        <div className="w-full h-full flex flex-col items-center justify-center bg-black relative overflow-hidden">
            ...
            >
                <div className="bg-[#1a1a1a] border border-white/10 rounded-[2rem] p-4 flex justify-between shadow-3xl items-end relative">
                    {modelsError && (
                        <div className="absolute -top-11 left-0 right-0 mx-auto max-w-md px-4 py-2 rounded-xl bg-red-500/10 border border-red-500/20 text-red-300 text-[10px] font-medium text-center">
                    ...
                    <div className="flex items-center gap-2 h-full self-end mb-1">
                        ...
                        <button
                            ...
                            disabled={isGenerating || !settings.prompt.trim() || !imageModel}
